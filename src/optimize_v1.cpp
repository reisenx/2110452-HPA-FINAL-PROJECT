/**
 * @file optimize_v1.cpp
 * @author Worralop Srichainont
 * @brief Minimum Power Plant Coverage solution code with dynamic power coverage tracking.
 * @version 2.0
 * @date 12 Apr 2026
 */
...
void place_power_plant(int current_node, int current_n_power_plant)
{
    ...
    // Update power coverage status for the current node
    if(is_covered[current_node] == 0)
    {
        n_node_covered++;
    }
    is_covered[current_node]++;

    // Update power coverage status for the adjacent node
    for(int &adj_node : adj_list[current_node])
    {
        if(is_covered[adj_node] == 0)
        {
            n_node_covered++;
        }
        is_covered[adj_node]++;
    ...
    // Backtracking for the current node
    ...
    // Backtracking for the adjacent node
    for(int &adj_node : adj_list[current_node])
    {
        is_covered[adj_node]--;
        if(is_covered[adj_node] == 0)
