/**
 * @file optimize_v3.cpp
 * @author Worralop Srichainont
 * @brief Minimum Power Plant Coverage solution code with greedy algorithm.
 * @version 4.0
 * @date 12 Apr 2026
 */
...
struct uint128_t
{
    ...
    // Utility function to count amount of 1 bits
...
void setup_greedy_solution(uint128_t initial_is_not_covered)
{
    ...
    while(!is_not_covered.is_zero())
    {
        ...
        {
            int n_covered = (is_not_covered & adj_matrix[i]).count_bits();
        ...
        greedy_solution[best_node] = 1;
        greedy_min_n_power_plant++;

        // Update power coverage status
        is_not_covered = is_not_covered & ~adj_matrix[best_node];
    }
...
int main(int argc, char* argv[])
{
    ...
    // Create the initial state where every node is not covered
